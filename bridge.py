...
def _make_ownship_report(override: dict | None = None) -> bytes:
    """Build one GDL90 Ownship Report frame using the current ownship position.
    If *override* is provided, use that aircraft's data instead of GPS."""
    if override is not None:
        lat = override.get("latitude", _OWNSHIP_DEFAULT_LAT)
        lon = override.get("longitude", _OWNSHIP_DEFAULT_LON)
        alt_ft = override.get("altitude", _OWNSHIP_DEFAULT_ALT)
        track_deg = override.get("track")
        speed_kt = override.get("ground_speed")
        heading_deg = None
    else:
        with _ownship_lock:
            lat = _ownship["lat"]
...
def _make_ownship_geo_alt(override: dict | None = None) -> bytes:
    """Build one GDL90 Ownship Geometric Altitude frame."""
    if override is not None:
        alt_geo_ft = override.get("altitude", _OWNSHIP_DEFAULT_ALT)
    else:
        with _ownship_lock:
            ...
            while True:
                ...
                for line in text.splitlines():
                    ...
                    try:
                        ...
                        pass
                    # Skip writing ownship.json when override is active —
                    # the traffic thread writes the aircraft's position instead.
                    if _cfg.get("ownship", "icao").strip():
                        continue
                    try:
                        with _ownship_lock:
                            snap = {
    ...
    while True:
        ...
        try:
            ...
            while True:
                ...
                # Check for ownship aircraft override from config
                override_icao = _cfg.get("ownship", "icao", reload=True).strip().upper()
                override_data = None
                if override_icao:
                    with _aircraft_lock:
                        ac = _aircraft.get(override_icao)
                        if ac and "latitude" in ac and "longitude" in ac:
                            override_data = dict(ac)
                ...
                time.sleep(0.01)
                sock.sendto(_make_ownship_report(override_data), dest)
                time.sleep(0.01)
                sock.sendto(_make_ownship_geo_alt(override_data), dest)
                time.sleep(0.01)
                ...
                for icao, ac in snapshot.items():
                    ...
                    # Skip the ownship override aircraft from traffic
                ...
                # When ownship override is active, update ownship.json so
                # radar.py uses the overridden position as center.
                if override_data is not None:
                    try:
                        snap = {
                            "lat": override_data.get("latitude", 0.0),
                            "lon": override_data.get("longitude", 0.0),
                            "alt_ft": override_data.get("altitude", 0.0),
                            "track_deg": override_data.get("track"),
                            "speed_kt": override_data.get("ground_speed"),
                            "has_fix": True,
                        }
                        tmp = OWNSHIP_JSON_PATH + ".tmp"
                        with open(tmp, "w") as f:
